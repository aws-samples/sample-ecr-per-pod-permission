...
export IRSA_ROLE_ARN_TEAM_A=$(terraform -chdir="$TF_DIR" output -raw irsa_role_arn_team_a)
export IRSA_ROLE_ARN_TEAM_B=$(terraform -chdir="$TF_DIR" output -raw irsa_role_arn_team_b)

ENVSUBST_VARS='${ACCOUNT_ID} ${REGION} ${CLUSTER_NAME} ${ECR_SHARED_REPO_URL} ${ECR_TEAM_A_REPO_URL} ${ECR_TEAM_B_REPO_URL} ${ECR_BASELINE_REPO_URL} ${IRSA_ROLE_ARN_TEAM_A} ${IRSA_ROLE_ARN_TEAM_B}'
...
echo "IRSA_ROLE_ARN_TEAM_A=$IRSA_ROLE_ARN_TEAM_A"
echo "IRSA_ROLE_ARN_TEAM_B=$IRSA_ROLE_ARN_TEAM_B"
...
if [ "$ACTION" = "apply" ]; then
  ...
  for f in "$MANIFEST_DIR"/team-*.yaml "$MANIFEST_DIR"/no-ecr-irsa-*.yaml; do
